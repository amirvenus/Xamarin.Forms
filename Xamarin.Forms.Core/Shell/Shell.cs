...
namespace Xamarin.Forms
{
	...
	public class Shell : Page, IShellController, IPropertyPropagationController
	{
		...
		ObservableCollection<ObservableCollection<Element>> _flyoutItems = new ObservableCollection<ObservableCollection<Element>>();

		internal void SendStructureChanged()
		{
			_flyoutItems.Clear();
			if (this is IShellController controller)
			{
				foreach (var items in controller.GenerateFlyoutGrouping())
				{
					ObservableCollection<Element> elements = new ObservableCollection<Element>();
					_flyoutItems.Add(elements);
		...
		internal override IEnumerable<Element> ChildrenNotDrawnByThisElement
		{
			get
			{
				...
				foreach (var items in _flyoutItems)
				{
					foreach (var item in items)
					{
						if (item is MenuShellItem msi)
							yield return msi.MenuItem;
